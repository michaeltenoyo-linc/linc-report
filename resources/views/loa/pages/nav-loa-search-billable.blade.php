        ...
        <div class="p-8 ">
            <form id="form-search-billable" autocomplete="off" enctype="multipart/form-data">
                ...
                <div class="w-full lg:w-12/12 px-4">
                    <div class="relative w-full mb-3">
                        <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2"
                            htmlFor="name">Unit Type</label>
                        
                        <select required id="unit" name="unit" class="input-route-start border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150" type="text">
                            <option value="-1">==Semua==</option>
                            @foreach ($units as $u)
                                <option value="{{ $u->sku ?? '' }}">{{ $u->sku ?? '' }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
